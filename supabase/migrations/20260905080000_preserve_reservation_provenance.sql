-- ============================================================================
-- MIGRACIÓN: Preservar trazabilidad de procedencia (purchase_item_id) en reservas
-- ============================================================================
-- Al recibir mercadería y convertir reservas de 'incoming' a 'physical',
-- se preserva el purchase_item_id original en lugar de anularlo.
-- Asimismo, la fusión de reservas físicas se restringe a aquellas que compartan
-- el mismo origen de compra (o ambas provenientes de stock inicial sin compra).

create or replace function public.receive_purchase(p_purchase_id uuid, p_items jsonb)
returns jsonb
language plpgsql
security definer
