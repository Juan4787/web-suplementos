-- ============================================================================
-- MIGRACIÓN: Concurrencia de Idempotencia y Contrato Material Completo
-- ============================================================================
-- 1. Anti-Race en Doble Submit Simultáneo:
--    Usa pg_advisory_xact_lock(hashtext(v_protocol_order_id::text)) para que dos
--    requests concurrentes con la misma clave se serialicen limpiamente.
--    El segundo request ve la orden ya confirmada y devuelve el mismo order_id
--    sin arrojar unique_violation ni 500.
-- 2. Validación de Contrato Material Completo:
--    Verifica de forma autoritativa en servidor que no haya cambiado ningún campo:
--    customerName, phone, paymentMethod, deliveryMethod, shippingType,
--    shippingAddress, shippingFee, además de productos, cantidades y checksum.
--    Cualquier diferencia arroja IDEMPOTENCY_KEY_REUSE_MISMATCH.
...
begin
  ...
  -- 1. Idempotencia Concurrente Segura (Serialización vía Advisory Lock por Clave)
  if v_protocol_order_id is not null then
    ...
    if v_existing_order.id is not null then
      -- 1a. Contrato Material Completo: Comparar campos de negocio directamente en DB
      ...
      -- 1b. Comparar cantidad de líneas, productos y cantidades exactas
      ...
      -- Reintento idéntico legítimo confirmado
