-- ============================================================================
-- MIGRACIÓN: Idempotencia en Recepciones de Compra (purchase_receipts)
-- ============================================================================
-- Permite recibir mercadería con una clave de idempotencia 'p_operation_id'.
-- Si un cliente reintenta la misma recepción parcial por timeout de red,
-- la base de datos detecta la operación previa y retorna el resultado exacto
-- sin duplicar stock, reservas ni asientos contables.
...
create index if not exists idx_purchase_receipts_operation 
  on public.purchase_receipts(operation_id);

create index if not exists idx_purchase_receipts_purchase 
  on public.purchase_receipts(purchase_id);

alter table public.purchase_receipts enable row level security;

create policy purchase_receipts_owner_all on public.purchase_receipts
...
-- Eliminar versión anterior de dos parámetros para unificar con default null
...
begin
  ...
  -- 3. ASENTAR RECIBO IDEMPOTENTE
  if p_operation_id is not null then
    insert into public.purchase_receipts (
      operation_id, purchase_id, items_payload, result_payload, created_by
    ) values (
      p_operation_id, p_purchase_id, p_items, v_res_obj, auth.uid()
    )
