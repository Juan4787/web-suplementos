-- Fix: Corrección del estado de liberación de reservas en transition_order('cancel')
-- El enum reservation_state admite ('active', 'consumed', 'released'). Se corrige el uso erróneo de 'cancelled' por 'released'.
-- Se asegura la restitución de stock y estados de fulfillment_state = 'cancelled' y movimiento 'reservation_release'.
