-- ============================================================================
-- MIGRACIÓN: Validación estricta de Idempotency Key Reuse Mismatch en confirm_imported_order
-- ============================================================================
-- Si un cliente envía un 'protocolOrderId' ya registrado previamente pero
-- con líneas diferentes (cantidad, producto) o checksum inconsistente,
-- la función rechaza la operación inmediatamente con 'IDEMPOTENCY_KEY_REUSE_MISMATCH'.
-- Si el payload es idéntico (reintento seguro), devuelve la orden existente.
...
begin
  ...
  -- 1. Idempotencia UX Real: Si ya existe, validar consistencia estricta
  if v_protocol_order_id is not null then
    select id, protocol_checksum, customer_name_snapshot, payment_method, delivery_method,
    ...
    if v_existing_order.id is not null then
      -- 1a. Si el checksum difiere del registrado en el pedido:
      if v_checksum is not null and v_checksum <> '' and v_existing_order.protocol_checksum is not null and v_existing_order.protocol_checksum <> v_checksum then
        raise exception using errcode = 'P0001', message = 'IDEMPOTENCY_KEY_REUSE_MISMATCH';
      end if;

      -- 1b. Si difieren las líneas (cantidad de ítems, productos o cantidades):
