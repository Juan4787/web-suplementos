...
export const buildBusinessWorkbook = (data: ExportDataset): BusinessWorkbook => {
  ...
  const sheets: WorkbookSheet[] = [
    ...
    sheet('Detalle compras', ['Ítem ID', 'Compra ID', 'Compra número', 'Producto ID', 'Producto', 'Cantidad pedida', 'Cantidad recibida', 'Faltante', 'Costo unitario ARS'], [38, 38, 14, 38, 28, 16, 16, 14, 20], data.purchases.flatMap((purchase) => purchase.items.map((item) => [
      text(item.id), text(purchase.id), number(purchase.number), text(item.productId), text(item.productName), number(item.quantity), number(item.receivedQuantity ?? 0), number(item.shortageQuantity ?? 0), pesos(item.unitCostCents)
    ]))),
