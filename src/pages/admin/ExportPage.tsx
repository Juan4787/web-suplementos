  ...
  return (
    <RoleGate capability="export_data">
      <div className="page-enter">
        <PageHeader title="Exportar todos mis datos" description="Una planilla de Excel completa con tus productos, stock, ventas, compras, gastos y clientes." />
        <div className="grid gap-6 xl:grid-cols-[1fr_22rem]">
          <section className="rounded-[2rem] bg-white p-6 shadow-card sm:p-8">
            ...
            <h2 className="mt-6 font-display text-3xl font-black">Un archivo. Quince hojas.</h2>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{['Resumen', 'Productos', 'Stock', 'Pedidos', 'Detalle pedidos', 'Ventas', 'Compras', 'Detalle compras', 'Movimientos', 'Reservas', 'Clientes', 'Gastos varios', 'Historial de gastos', 'Índices y ajustes', 'Usuarios'].map((sheet) => <div key={sheet} className="flex items-center gap-2 rounded-2xl bg-cream-100 px-4 py-3 text-sm font-bold"><CheckCircle2 className="size-4 text-brand-600" /> {sheet}</div>)}</div>
