  ...
  return (
    <RoleGate capability="export_data">
      <div className="page-enter">
        <PageHeader title="Exportar todos mis datos" description="Un respaldo tabular para recuperar productos, pedidos, ventas, compras, stock, movimientos y clientes." />
        <div className="grid gap-6 xl:grid-cols-[1fr_22rem]">
          <section className="rounded-[2rem] bg-white p-6 shadow-card sm:p-8">
            ...
            <p className="mt-3 max-w-2xl leading-7 text-ink-600">Cada relación conserva sus IDs y sus valores históricos. Los textos se escriben como texto, nunca como fórmulas, incluso si comienzan con signos especiales.</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{['Resumen', 'Productos', 'Stock', 'Pedidos', 'Detalle pedidos', 'Ventas', 'Compras', 'Detalle compras', 'Movimientos', 'Reservas', 'Clientes', 'IPC oficial', 'Usuarios'].map((sheet) => <div key={sheet} className="flex items-center gap-2 rounded-2xl bg-cream-100 px-4 py-3 text-sm font-bold"><CheckCircle2 className="size-4 text-brand-600" /> {sheet}</div>)}</div>
          ...
          <aside className="space-y-5">
            <section className="rounded-[1.75rem] bg-ink-950 p-5 text-white"><ShieldCheck className="size-6 text-brand-300" /><h2 className="mt-4 font-display text-xl font-black">Acceso protegido</h2><p className="mt-2 text-sm leading-6 text-white/60">El servidor decide qué datos puede exportar la sesión. El navegador solo construye el archivo autorizado.</p></section>
            <section className="rounded-[1.75rem] border border-amber-200 bg-amber-50 p-5"><ImageOff className="size-6 text-amber-800" /><h2 className="mt-4 font-display text-xl font-black">Qué no incluye</h2><p className="mt-2 text-sm leading-6 text-amber-950/70">No copia las imágenes binarias, el código, migraciones SQL, usuarios de Auth ni secretos. Es un respaldo de datos comerciales, no una imagen completa de la infraestructura.</p></section>
          </aside>
        </div>
      </div>
    </RoleGate>
