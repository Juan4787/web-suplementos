...
it.each(['entrega', 'cancelación'])('confirma %s fuera de la tarjeta y permite encontrar el pedido nuevamente', async variant => {
  ...
  api.transitionOrder.mockImplementation(async () => { order = variant === 'entrega' ? { ...order, fulfillmentState: 'delivered' } : { ...order, orderState: 'cancelled' }; return order; });
  ...
  expect(await screen.findByText(`Pedido #${order.number} ${variant === 'entrega' ? 'completado' : 'cancelado'}. Lo encontrás en Completados.`)).toBeInTheDocument();
