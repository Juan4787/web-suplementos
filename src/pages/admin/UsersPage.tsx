  ...
  return (
    <RoleGate capability="manage_users">
      <div className="page-enter">
        <PageHeader title="Usuarios y permisos" description="Administrá los accesos y roles de tu equipo de trabajo." />
        ...
        <div className="space-y-4">{usersQuery.data?.map((user) => {
          ...
          return (
            <article
              key={user.id}
              className="grid gap-4 rounded-[1.75rem] bg-white p-5 shadow-card sm:grid-cols-[auto_1fr_12rem_10rem_auto] sm:items-center sm:p-6"
            >
              <span
                className={`grid size-12 place-items-center rounded-2xl ${
                  user.role === 'owner' ? 'bg-brand-600 text-white' : 'bg-cream-100 text-ink-800'
                }`}
              >
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  />
                  <StatusChip
                    label={user.active ? 'Habilitada' : 'Pendiente'}
              ...
              <Select
                ...
                onChange={(event) =>
                  setDraftActive((current) => ({
                    ...current,
                    [user.id]: event.target.value === 'active'
                  }))
                }
              >
        ...
        <p className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">Para agregar a una persona, solicitá la creación de su cuenta con el correo que va a usar. Después aparecerá aquí para que elijas su rol y habilites el acceso.</p>
      </div>
    </RoleGate>
  );
}
